...
void solve() {
    // There's 2 possibilities, either go clockwise or anti-clockwise and
    // determine the minimum of the two.
    int n;
    cin >> n;
    set<pair<int, int>> original;
    vt<vt<pair<int, int>>> adj(n + 1);
    vt<bool> vis(n + 1, false);
    int64_t total = 0, clockwise = 0, anti = 0;
    FOR(n) {
        int u, v, c;
        cin >> u >> v >> c;
        adj[u].pb({v, c});
    ...
    function<void(int, int, int64_t &)> dfs = [&](int node, int par,
                                                  int64_t &tt) {
        if (vis[node])
            return;
        vis[node] = true;
        for (auto &[next, cost] : adj[node]) {
            if (!vis[next] || (next == 1 && par != 1)) {
                if (!original.contains({node, next})) {
                    tt += cost;
                }
                dfs(next, node, tt);
            }
    ...
    dfs(1, -1, clockwise);
    anti = total - clockwise;
    cout << min(anti, clockwise) << '\n';
}
