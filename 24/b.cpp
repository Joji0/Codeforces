...
vt<map<string, int>> places(70);

void conquer(vt<string> &v, int compareAt) {
    vt<string> temp;
    int maxx = 0;
    for (auto &[name, cnt] : places[compareAt]) {
        if (find(all(v), name) != v.end()) {
            chmax(maxx, cnt);
        }
    }
    for (auto &[name, cnt] : places[compareAt]) {
        if (cnt == maxx && find(all(v), name) != v.end()) {
            temp.pb(name);
        }
    }
    if (!temp.empty())
...
void solve() {
    ...
    FOR(t) {
        ...
        FOR(n) {
            string win;
            cin >> win;
            mp[win] += (i < 10 ? score[i] : 0);
            places[i][win]++;
        }
    }
    vt<string> candidates;
    int maxpoints = 0;
    for (auto &[p, points] : mp) {
        chmax(maxpoints, points);
    }
    for (auto &[p, points] : mp) {
        if (points == maxpoints)
            candidates.pb(p);
    ...
    int comp = 0;
    while (sz(candidates) > 1 && comp < 60) {
        conquer(candidates, comp);
        comp++;
    }
    cout << candidates.front() << '\n';
    candidates.clear();
    int maxwins = 0;
    for (auto &[p, points] : places[0]) {
        chmax(maxwins, points);
    }
    for (auto &[p, points] : places[0]) {
        if (points == maxwins) {
            candidates.pb(p);
    ...
    if (candidates.size() == 1) {
        ...
        maxpoints = 0;
        for (auto &[p, points] : mp) {
            if (find(all(candidates), p) != candidates.end())
                chmax(maxpoints, points);
        }
        for (auto &[p, points] : mp) {
            if (find(all(candidates), p) != candidates.end() &&
