...
void solve() {
    // idea : take power up only if there exists a hurdle in which current jump
    // power doesnt suffice.
    int n, m;
    int64_t L;
    cin >> n >> m >> L;
    vt<array<int64_t, 2>> hurdle(n), pow(m);
    priority_queue<int64_t> bestPow;
    FOR(n) { cin >> hurdle[i][0] >> hurdle[i][1]; }
    FOR(m) { cin >> pow[i][0] >> pow[i][1]; }
    int64_t jump = 1;
    int ptrH = 0, ptrP = 0, ans = 0;
    while (ptrH < n) {
        while (ptrP < m && (pow[ptrP][0] <= hurdle[ptrH][0])) {
            // lets pickup a powerup along the way before our first hurdle
            // but we can just store them in our bestPow pq instead of using it
            // right away
            bestPow.push(pow[ptrP][1]);
            ptrP++;
        }
        auto [l, r] = hurdle[ptrH];
        while (!bestPow.empty() && jump < r - l + 2) {
            // use the powerups as needed
            jump += bestPow.top();
            bestPow.pop();
            ans++;
        }
        if (jump < r - l + 2) {
            // still cant jump after using every power then fail
            break;
        }
        ptrH++;
    }
