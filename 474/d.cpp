...
const int64_t maxN = 100005, MOD = 1e9 + 7;

int64_t dp[maxN], pref[maxN];

void precompute(const int &size, const int &k) {
    dp[0] = 1;
    FOR(i, 1, size) {
        dp[i] = (dp[i - 1] % MOD + (i - k >= 0 ? dp[i - k] % MOD : 0)) % MOD;
    }
    pref[0] = 0;
    FOR(i, 1, size) { pref[i] = (pref[i - 1] % MOD + dp[i] % MOD) % MOD; }
}
