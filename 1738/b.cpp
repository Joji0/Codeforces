...
void solve() {
    // We will first recover the original suffix of the array with length k.
    // After that we check whether we can construct non-decreasing original
    // array from our current suffix.
    int n, k;
    cin >> n >> k;
    if (k == 1) {
        int64_t trash;
    ...
    vt<int64_t> pref(k), arr(k - 1);
    cin >> pref;
    FOR(k - 1) { arr[i] = pref[i + 1] - pref[i]; }
    FOR(i, 1, k - 1) {
        if (arr[i] < arr[i - 1]) {
            cout << "NO\n";
            return;
