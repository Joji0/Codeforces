...
struct StringHash {
    ...
    int count_unique_substring() const {
        int cnt = 0;
        FOR(len, 1, n + 1) {
...
void solve() {
    // we need to find string t that is the prefix, suffix, and contained in the
    // middle of s.
    string s;
    cin >> s;
    int n = sz(s);
    StringHash SH(s);
    vt<int64_t> candidates;
    FOR(i, 1, n) {
        if (SH.get(0, i - 1) == SH.get(n - i, n - 1)) {
            candidates.pb(i);
        }
    }
    ...
    EACH(cand, candidates) {
        FOR(i, 1, n - cand) {
            if (SH.get(i, i + cand - 1) == SH.get(0, cand - 1)) {
