...
void solve() {
    ...
    FOR(k) {
        ...
        vt<int> cur(n);
        cin >> cur;
        int64_t accum = accumulate(all(cur), LL(0));
        FOR(j, n) {
            if (mp.contains(accum - cur[j]) &&
                mp[accum - cur[j]].first != i + 1) {
                cout << "YES\n";
                cout << i + 1 << " " << j + 1 << '\n';
                cout << mp[accum - cur[j]].first << " "
                     << mp[accum - cur[j]].second << '\n';
                return;
            }
            mp[accum - cur[j]] = {i + 1, j + 1};
        }
    }
    cout << "NO\n";
}
