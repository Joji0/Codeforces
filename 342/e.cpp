...
struct CentroidDecomposition {
    ...
    template <class F> void for_each(int node, F &&f) {
        for (auto &s : chain[node]) {
...
void solve() {
    ...
    centroid.build();
    auto upd = [&](int &c, int &dist, int &_) -> void { chmin(best[c], dist); };
    centroid.for_each(0, upd);
    while (M--) {
        int t, v;
        cin >> t >> v;
        v--;
        if (t == 1) {
            centroid.for_each(v, upd);
        } else {
            int ans = INF;
            for (auto &[centr, dist, child] : centroid.chain[v]) {
                chmin(ans, best[centr] + dist);
            }
            cout << ans << '\n';
        }
    }
