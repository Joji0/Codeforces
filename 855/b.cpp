...
void solve() {
    ...
    vt<int64_t> A(n);
    cin >> A;
    static int64_t memo[maxN][3];
    memset(memo, -1, sizeof(memo));
    vt<int64_t> opt = {p, q, r};
    vt<int64_t> TransformedA;
    TransformedA.pb(A[0]);
    int ptr = 1;
    while (ptr < n) {
        while (ptr < n && A[ptr] == TransformedA.back()) {
            ptr++;
        }
        if (ptr < n) {
            TransformedA.pb(A[ptr]);
        }
    }
    function<int64_t(int, int)> dp = [&](int idx, int chosen) -> int64_t {
        if (chosen >= 3) {
            return 0;
        }
        if (idx >= sz(TransformedA)) {
            return -INF;
        }
        if (memo[idx][chosen] != -1) {
            return memo[idx][chosen];
        }
        int64_t res = -INF;
        if (dp(idx, chosen + 1) != -INF) {
            chmax(res, dp(idx, chosen + 1) + TransformedA[idx] * opt[chosen]);
        }
        if (dp(idx + 1, chosen) != -INF)
            chmax(res, dp(idx + 1, chosen));
        return memo[idx][chosen] = res;
    };
    cout << dp(0, 0) << '\n';
}
