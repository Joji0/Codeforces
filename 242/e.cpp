...
struct Node {
    vt<int> b;
    Node(int n = BIT, int val = 0) : b(n, val) {}
};

Node merge(Node a, Node b) {
    Node base;
    FOR(BIT) { base.b[i] = a.b[i] + b.b[i]; }
    return base;
}

int64_t calc(const Node &a) {
    int64_t ans = 0;
    FOR(BIT) { ans += 1LL * a.b[i] * (1 << i); }
    return ans;
}
...
struct SegmentTree {
    ...
    void push(int v, int tl, int tr) {
        ...
        int tm = (tl + tr) / 2;
        FOR(BIT) {
            int setBit = lazy[v].b[i];
            if (setBit) {
                t[v * 2].b[i] = (tm - tl + 1) - (t[v * 2].b[i]);
                t[v * 2 + 1].b[i] = (tr - tm) - (t[v * 2 + 1].b[i]);
                lazy[v * 2].b[i] ^= 1;
                lazy[v * 2 + 1].b[i] ^= 1;
            }
        }
    ...
    int64_t sum(int l, int r) { return sum(1, 0, n - 1, l, r); }

    void update(int v, int tl, int tr, int l, int r, int64_t x) {
        if (l > r)
            return;
        if (l == tl && r == tr) {
            FOR(BIT) {
                if ((x >> i) & 1) {
                    t[v].b[i] = (tr - tl + 1) - t[v].b[i];
                    lazy[v].b[i] ^= 1;
                }
            }
        } else {
    ...
    void update(int l, int r, int64_t add) { update(1, 0, n - 1, l, r, add); }
};
