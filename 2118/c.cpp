...
struct cmp {
    bool operator()(int64_t a, int64_t b) {
        int fa = firstUnset(a), fb = firstUnset(b);
        return fa > fb;
    }
};

void solve() {
    // idea: use min-heap and try to add +1 until some number X > curr, where X
    // have the smallest 0 bit in the current number to be set to 1
    int n;
    int64_t k;
    cin >> n >> k;
    priority_queue<int64_t, vt<int64_t>, cmp> pq;
    FOR(n) {
        int64_t x;
        cin >> x;
        pq.push(x);
    ...
    while (k > 0) {
        int64_t curr = pq.top();
        pq.pop();
        int idx = firstUnset(curr);
        int64_t got = curr | (1LL << idx), pay = (1LL << idx);
        if (k < pay) {
            // break if the current k wont even suffice for the smallest element
            pq.push(curr);
            break;
        } else {
            k -= pay;
