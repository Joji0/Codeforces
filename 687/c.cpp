...
void solve() {
    ...
    FOR(idx, 1, n + 1) {
        FOR(sum, k + 1) {
            FOR(sumSubset, k + 1) {
                dp[idx][sum][sumSubset] |= dp[idx - 1][sum][sumSubset];
                if (sum - c[idx - 1] >= 0 && sumSubset - c[idx - 1] >= 0)
                    dp[idx][sum][sumSubset] |=
                        dp[idx - 1][sum - c[idx - 1]][sumSubset - c[idx - 1]];
                if (sum - c[idx - 1] >= 0)
                    dp[idx][sum][sumSubset] |=
                        dp[idx - 1][sum - c[idx - 1]][sumSubset];
            }
        }
    }
    vt<int> ans;
