...
void solve() {
    int64_t N, K;
    cin >> N >> K;
    vector<Mint> dp(K + 2, 0), pref(K + 2, 1), suff(K + 3, 1);
    for (int i = 1; i <= K + 1; i++) {
        dp[i] = dp[i - 1] + (Mint)(mod_pow(i, K));
    }
    for (int i = 0; i <= K + 1; i++) {
        pref[i] = (i ? pref[i - 1] * Mint(N - i) : Mint(N - i));
    }
    for (int i = K + 1; i >= 0; i--) {
        suff[i] = suff[i + 1] * (N - i);
    }
    if (N <= K + 1) {
        ...
        Mint ans = 0;
        for (int i = 0; i <= K + 1; i++) {
            ans += dp[i] * (i ? pref[i - 1] : 1) * (suff[i + 1]) *
                   ((K + 1 - i) & 1 ? -1 : 1) * invfact[K + 1 - i] * invfact[i];
        }
        cout << ans << '\n';
    }
}
