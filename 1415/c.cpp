...
void solve() {
    // Define pref[i] as the cost to fix cell i and fill this from the back (n).
    int n, p, k, x, y;
    string s;
    cin >> n >> p >> k >> s >> x >> y;
    vt<int64_t> pref(n, 0);
    pref[n - 1] = (s[n - 1] == '0' ? x : 0);
    FOR(i, n - 2, -1, -1) {
        pref[i] = (s[i] == '0' ? x : 0) + (i + k < n ? pref[i + k] : 0);
    }
    int64_t ans = 1e18;
    // Try to start at every cell from p to n by deleting with cost of y.
    FOR(i, p - 1, n) {
        int64_t cost = (i - p + 1) * y;
        chmin(ans, pref[i] + cost);
    }
    cout << ans << '\n';
}
