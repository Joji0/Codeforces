...
const int maxN = 5005, maxK = 5005, INF = 1e9;

int64_t memo[maxN][maxK], pref[maxN];

int64_t dp(int pos, int split, const int &len) {
    if (split == 0)
        return 0;
    if (pos <= 0)
        return -INF;
    ...
    int64_t ans = dp(pos - 1, split, len);
    if (pos >= len) {
        chmax(ans, dp(pos - len, split - 1, len) + pref[pos] - pref[pos - len]);
    }
    return memo[pos][split] = ans;
}
...
void solve() {
    ...
    cin >> A;
    pref[1] = A[0];
    FOR(i, 2, n + 1) { pref[i] = pref[i - 1] + A[i - 1]; }
