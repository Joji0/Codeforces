...
void solve() {
        ...
        // swap can make either decrease dis by 2, by 1, or none.
        // dec by 2 case
        for (int i = 0; i < N; i++) {
                if (S[i] != T[i]) {
                        if (need_by_T.contains(S[i]) && S[need_by_T[S[i]]] == T[i]) {
                                // swap this two guys
                                cout << ham - 2 << '\n';
                                cout << i + 1 << " " << need_by_T[S[i]] + 1 << '\n';
                                return;
                        }
                }
        }
