...
void solve() {
    // Observe that there's a neat trick for this problem. The answer is 0 <=
    // operation <= 2. This is because if all the characters are already c, then
    // we are done (0). Else, we first brute force iterate from 1 to n and check
    // if there exists a number x such that if we pick x, we are done (1). Worst
    // case, we do x = n and x = n - 1 the reason being this will always make
    // our string all filled with c.
    int n;
    char c;
    string s;
    cin >> n >> c >> s;
