...
bool ok(int l, int mid, int r) { return (l <= mid && mid <= r); }
...
void solve() {
    ...
    vt<int64_t> A(N);
    cin >> A;
    vt<vt<int64_t>> dp(N + 1, vt<int64_t>(H, -1));
    dp[0][0] = 0;
    FOR(i, N) {
        FOR(h, H) {
            if (dp[i][h] == -1)
                continue;
            int64_t t1 = (h + A[i]) % H, t2 = (h + A[i] - 1 + H) % H;
            chmax(dp[i + 1][t1], dp[i][h] + ok(l, t1, r));
            chmax(dp[i + 1][t2], dp[i][h] + ok(l, t2, r));
        }
    }
