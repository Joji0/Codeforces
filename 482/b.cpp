...
void solve() {
    ...
    cin >> n >> m;
    vt<int> A(n, 0);
    vt<vt<int>> diff(n, vt<int>(30, 0));
    vt<array<int, 3>> constraints(m);
    FOR(m) {
        FOR(j, 3) { cin >> constraints[i][j]; }
        auto [l, r, q] = constraints[i];
        FOR(j, 30) {
            if ((1 << j) & q) {
                diff[l - 1][j] += 1;
                if (r < n)
    ...
    FOR(j, 30) {
        vt<int> origin(n, 0);
        FOR(n) {
            origin[i] = (i ? origin[i - 1] : 0) + diff[i][j];
            if (origin[i] > 0) {
                A[i] = A[i] | (1 << j);
            }
        }
