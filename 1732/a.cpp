...
void solve() {
    // Observe that there's a neat trick. Suppose g = gcd(a[0]..a[n - 1]). Then
    // if g == 1 we return 0 because there's no need to do any operation. Else
    // we move greedily to the end of the array because it's cheapest to do
    // the operation there. Now g = gcd(a[0]..gcd(a[n - 1], n)). But then this
    // will be reduced to g = gcd(g, n). If this is 1, we return 1 and done.
    // Else, we move to the next cheapest place to do the operation, index n
    // - 1. Again we will get g = gcd(g, n - 1) this time, if this is enough to
    // get g = 1, we stop and return 2. Else we return 3, this is from the fact
    // that if we change at index n and n - 1 we will always get gcd(n, n - 1) =
    // 1 hence we have 1 + 2 operations = 3.
    int n;
    cin >> n;
    vt<int> a(n);
    cin >> a;
