...
vector<map<string, int>> encountered;
vector<int> depth;
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        cin >> name >> p;
        p--;
        if (!get_id.contains(name)) {
            int id = (int)get_id.size();
            get_id[name] = id;
        }
        names[i] = get_id[name];
        if (p != -1) {
    ...
    vector<int> distinct(mxd + 67, 0);
    map<int64_t, int> freq;
    auto custom_key = [&](int dep, int nm) -> int64_t {
        return ((int64_t)dep << 20) ^ (int64_t)nm;
    };
    auto add = [&](const int &node) {
        if (node == SUPER)
            return;
        int64_t key = custom_key(depth[node], names[node]);
        int &f = freq[key];
        if (f == 0)
            distinct[depth[node]]++;
        f++;
