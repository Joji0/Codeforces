...
template <class ElType, class N> struct SegmentTree {
    ...
    SegmentTree(int n, N ID, function<N(const N &, const N &)> merge,
                function<N(const ElType &)> make)
        : n(n), ID(ID), merge(merge), make(make) {
        assert(n > 0);
        t.assign(4 * n, ID);
    }
...
void solve() {
    ...
    vector<int> EMPTY(maxC, 0);
    auto merge = [&](const int &a, const int &b) -> int { return a + b; };
    auto make = [&](const int &a) -> int { return a; };
    seg.init(maxC, 0, merge, make);
    seg.build(EMPTY);
    byColor.assign(maxC, 0);
    auto f = [&](const int &POINT) -> int { return POINT + 1; };
    auto invf = [&](const int &POINT) -> int { return POINT - 1; };
    auto add = [&](const int &node) {
        int &szNow = byColor[C[node]];
        seg.modify(szNow, invf);
        szNow++;
        seg.modify(szNow, f);
    };
    auto rem = [&](const int &node) {
        int &szNow = byColor[C[node]];
        seg.modify(szNow, invf);
        szNow--;
        seg.modify(szNow, f);
    ...
    auto ans = [&](const int &node) {
        for (auto &[k, id] : Query[node]) {
            out[id] = seg.query(k, maxC - 1);
        }
    };
