...
void solve() {
    ...
    auto numerator = [&](const Mint &x) -> Mint {
        Mint ret = 1;
        for (int i = 0; i < D; i++) {
            ret *= Mint(x - i);
        }
    ...
    auto denumerator = [&](const int &x) -> Mint {
        Mint ret = 1;
        for (int i = 0; i < D; i++) {
            if (i == x)
                continue;
    ...
    for (int tr = D; tr < MOD; tr++) {
        Mint now = 0, Px = numerator(Mint(tr));
        for (int i = 0; i < D; i++) {
            now += query[i] * Px / Mint(tr - i) * denumerator(i);
        }
        if (!now.v) {
            cout << "! " << tr << endl;
            return;
