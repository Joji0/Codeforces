...
void generate(const string &s, int idx, string &curr, vt<string> &out) {
    if (1 <= sz(curr) && sz(curr) <= 4) {
        out.pb(curr);
    }
    if (sz(curr) == 4 || idx == sz(s))
        return;
    FOR(i, idx, sz(s)) {
        curr.pb(s[i]);
        generate(s, i + 1, curr, out);
        curr.pop_back();
    }
}
...
void solve() {
    ...
    Kuhn K(n, sz(Id));
    K.adj = Graph;
    int match = K.maxMatching();
    if (match != n) {
        cout << "-1\n";
        return;
    }
    vt<int> ans = K.getLeftMatching();
    EACH(an, ans) { cout << back[an] << '\n'; }
}
