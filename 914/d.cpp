...
template <class ElType, class N> struct SegmentTree {
    ...
    int count_bad(int v, int tl, int tr, int l, int r, ElType x) const {
        if (l > r)
            return 0;
        if (tl == l && tr == r) {
            if (t[v] % x == 0) {
        ...
        int tm = (tl + tr) / 2;
        int left = count_bad(v * 2, tl, tm, l, min(r, tm), x);
        if (left >= 2)
            return 2;
        int right = count_bad(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, x);
...
void solve() {
    ...
    vector<int64_t> A(N);
    cin >> A >> Q;
    auto merge = [&](const int64_t &a, const int64_t &b) { return gcd(a, b); };
    auto make = [&](const int64_t &a) { return a; };
    SegmentTree<int64_t, int64_t> seg(N, 0LL, merge, make);
    seg.build(A);
    while (Q--) {
        int t;
        cin >> t;
        if (t == 1) {
            ...
            cin >> l >> r >> x;
            l--, r--;
            int bad = seg.count_bad(1, 0, N - 1, l, r, x);
            cout << (bad >= 2 ? "NO\n" : "YES\n");
        } else {
            int i;
            int64_t y;
