...
void solve() {
        ...
        // check arithmetic
        int diff = b - a;
        if (a + 2 * diff == c && a + 3 * diff == d) {
                cout << a + 4 * diff << '\n';
        } else {
                // check geometric
                double ratio = 1.0 * b / a;
                auto same = [](double a, double b) -> bool { return abs(a - b) <= eps; };
                if (same(1.0 * a * ratio * ratio, 1.0 * c) && same(1.0 * a * ratio * ratio * ratio, 1.0 * d)) {
                        double got = 1.0 * a * ratio * ratio * ratio * ratio;
                        int gd = floor(got), gu = ceil(got);
                        if (same(got, gd)) {
                                cout << gd << '\n';
                        } else if (same(got, gu)) {
                                cout << gu << '\n';
