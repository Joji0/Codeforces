...
void solve() {
    // The idea is to use DP. Define dp[i][j] as the minimum possible total sum
    // ending at i-th position after using j operations. The answer is then just
    // minimum of all dp[n][j].
    int n, k;
    cin >> n >> k;
    vt<int64_t> a(n + 1);
    vt<vt<int64_t>> dp(n + 1, vt<int64_t>(k + 1, 1e18));
    FOR(i, 1, n + 1) cin >> a[i];
    dp[0][0] = 0;
    FOR(i, 1, n + 1) { dp[i][0] = dp[i - 1][0] + a[i]; }
    FOR(j, k + 1) {
        FOR(i, 1, n + 1) {
            // In case we haven't touched the index before this, we continue
            if (dp[i - 1][j] == 1e18)
                continue;
            // Possibilities: change the current index or not.
