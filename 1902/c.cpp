...
void solve() {
    // idea: gcd of difference between consecutive sorted elements?
    int n;
    cin >> n;
    vt<int64_t> A(n);
    cin >> A;
    sort(all(A));
    int64_t g = 0;
    FOR(n - 1) { g = gcd(g, A[i + 1] - A[i]); }
    if (n == 1) {
        cout << "1\n";
        return;
    }
    int64_t ans1 = 0, ans2 = 0, targ = A[n - 1] + g, bonus = 0;
    bool ok = true;
    FOR(n) {
        ans1 += (targ - A[i]) / g;
        if (ok) {
    ...
    int64_t pick = A[n - 1], i = n - 1;
    while (i >= 0 && pick == A[i]) {
        i--;
        pick -= g;
    }
    if (find(all(A), pick) != A.end())
        ok = false;
    bonus = (A[n - 1] - pick) / g;
    cout << min(ans1, ok ? (ans2 + bonus) : INT_MAX) << '\n';
}
