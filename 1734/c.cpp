...
void solve() {
    // The idea is to do sieving over the string T. Suppose we are at index i,
    // we will sieve while T[i] = 0, and set S[i] = 0 there. If we found a
    // multiple of i that is equal to 1, we stop because this will need to be
    // considered later if we reach that particular index and apply the same
    // sieving technique. Time complexity is O(n * log(n)) but this is safe
    // since the sum of n over all TC <= 1e6.
    int n;
    int64_t ans = 0;
    cin >> n;
    string T, S(n, '1');
