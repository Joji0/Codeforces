...
void solve() {
    int64_t n, d, lo, hi;
    cin >> n >> d >> lo >> hi;
    vt<array<int64_t, 3>> Cli(n);
    FOR(n) {
        cin >> Cli[i][0] >> Cli[i][1];
        Cli[i][2] = i + 1;
    }
    sort(all(Cli), [&](const auto &a, const auto &b) {
        return lo * a[0] + hi * a[1] < lo * b[0] + hi * b[1];
    });
    vt<int64_t> ans;
    FOR(n) {
        if (d < Cli[i][0] * lo + Cli[i][1] * hi) {
            break;
        } else {
            d -= Cli[i][0] * lo + Cli[i][1] * hi;
            ans.pb(Cli[i][2]);
        }
    }
    cout << sz(ans) << '\n';
    EACH(idx, ans) { cout << idx << " "; }
    cout << '\n';
}
