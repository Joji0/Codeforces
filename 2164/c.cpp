...
void solve() {
    ...
    cin >> n >> m;
    // vt<int64_t> sword(n);
    // priority_queue<int64_t> sword;
    multiset<int64_t> sword;
    vt<array<int64_t, 2>> mons(m);
    FOR(n) {
        int64_t s;
        cin >> s;
        sword.insert(s);
    ...
    EACH(s, sword) {
        while (ptr < m && mons[ptr][0] <= s) {
            pq.push({mons[ptr][1], mons[ptr][0]});
            ptr++;
        }
        if (pq.empty())
            continue;
        ...
        ans++;
        if (c > 0)
            sword.insert(max(s, c));
    }
    // FOR(m) {
    //     auto it = sword.lower_bound(mons[i][0]);
    //     if (it == sword.end()) {
    //         continue;
    //     }
    //     int64_t currSword = *it;
    //     sword.erase(it);
    //     ans++;
    //     if (mons[i][1] > 0)
    //         sword.insert(max(currSword, mons[i][1]));
    // }
    cout << ans << '\n';
}
