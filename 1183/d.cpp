...
void solve() {
    ...
    cin >> n;
    vt<int64_t> A(n);
    vt<pair<int64_t, int64_t>> freq(n + 1);
    cin >> A;
    FOR(n + 1) { freq[i].first = i; }
    EACH(x, A) { freq[x].second++; }
    sort(all(freq), [](const auto &a, const auto &b) {
        if (a.second == b.second) {
            return a.first < b.first;
        }
        return a.second > b.second;
    });
    int64_t ans = freq.front().second, curr = ans;
    FOR(i, 1, n + 1) {
        if (!freq[i].second || !curr)
            break;
        if (freq[i].second < curr) {
            ans += freq[i].second;
            curr = freq[i].second;
