...
void solve() {
        ...
        FOR(j, M) {
            if (g[i][j] == '#') {
                ...
                FOR(dir, 4) {
                    ...
                    if (ok(nx, ny)) {
                        int idNxt = id(nx, ny);
                        dsu.join(idCur, idNxt);
                    }
                }
            }
        }
    }
    vt<int> conRow(N, 0), conCol(M, 0);
    vt<int64_t> usedParent(N * M + 69, 0LL);
    int64_t timer = 0LL;
    FOR(N) {
        timer++;
        FOR(j, M) {
            for (int x : {-1, 0, 1}) {
                if (i + x < 0 || i + x >= N)
                    continue;
                if (g[i + x][j] == '#') {
                    int par = dsu.find(id(i + x, j));
                    if (usedParent[par] != timer) {
                        conRow[i] += dsu.sz[par];
                        usedParent[par] = timer;
                    }
                }
            }
        }
    ...
    int ans = 0;
    FOR(N) { chmax(ans, conRow[i] + r[i]); }
    FOR(j, M) { chmax(ans, conCol[j] + c[j]); }
    cout << ans << '\n';
    // debug(r);
    // debug(c);
}
