...
const int64_t MOD = 1e9 + 7, maxL = 3005, maxN = 105;
int64_t memo[maxL][maxN][maxN];
vt<array<int, 2>> Board;
int n, l;

int64_t dp(int prevWidth, int lastIdx, int curLen) {
    if (curLen > l) {
        return 0;
    } else if (curLen == l) {
        return 1;
    }
    if (memo[curLen][prevWidth][lastIdx] != -1) {
        return memo[curLen][prevWidth][lastIdx];
    }
    int64_t res = 0;
    if (prevWidth == 0) {
        FOR(i, 1, n + 1) {
            res += dp(Board[i][0], i, Board[i][1]) % MOD;
            res %= MOD;
            if (Board[i][0] != Board[i][1]) {
                res += dp(Board[i][1], i, Board[i][0]) % MOD;
                res %= MOD;
            }
        }
    } else {
        FOR(i, 1, n + 1) {
            if (i == lastIdx)
                continue;
            if (Board[i][1] == prevWidth) {
                res += dp(Board[i][0], i, curLen + Board[i][1]) % MOD;
                res %= MOD;
            }
            if (Board[i][0] != Board[i][1]) {
                if (Board[i][0] == prevWidth) {
                    res += dp(Board[i][1], i, curLen + Board[i][0]) % MOD;
                    res %= MOD;
                }
            }
        }
    }
    return memo[curLen][prevWidth][lastIdx] = res % MOD;
}

void solve() {
    cin >> n >> l;
    Board.resize(n + 1);
    FOR(i, 1, n + 1) { cin >> Board[i][0] >> Board[i][1]; }
