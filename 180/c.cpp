...
int64_t dp(int pos, int n, const string &s, int last) {
    if (pos == n) {
        if (last == 0 || last == 1) {
            return 0;
        } else {
            return (isCapital(s[pos]));
        }
    }
    if (memo[pos][last] != -1) {
    ...
    int64_t ans;
    bool change = isCapital(s[pos]);
    if (last == 0 || last == 1) {
        ans =
            min(dp(pos + 1, n, s, 1) + !change, dp(pos + 1, n, s, 2) + change);
    } else {
        ans = dp(pos + 1, n, s, 2) + change;
    }
