...
struct Hasher {
    const int64_t MOD = 1e9 + 7, BASE = 31;
    vt<int64_t> pref, pwr;
    ...
    int64_t get(int l, int r) {
        return (pref[r] - pref[l] * pwr[r - l] % MOD + MOD) % MOD;
    }
};
...
void solve() {
    ...
    int n = sz(f), m = sz(s);
    Hasher hf(f), hs(s);
    int64_t getS = hs.get(0, m);
    vt<int> ans;
    FOR(n) {
        int64_t l = hf.get(0, i), r = hf.get(i + 1, n),
                lr = (l * hf.pwr[n - 1 - i] + r) % hf.MOD;
        if (lr == getS) {
            ans.pb(i + 1);
        }
    }
