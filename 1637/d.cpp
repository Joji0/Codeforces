...
void solve() {
    ...
    int initDiff = abs(sumA - sumB); // minimize this maybe?
    FOR(n) {
        int nA = sumA - A[i] + B[i], nB = sumB - B[i] + A[i];
        if (abs(nA - nB) < initDiff) {
            initDiff = abs(nA - nB);
            sumA = nA;
            sumB = nB;
            swap(A[i], B[i]);
    ...
    auto count = [](vt<int> X) -> int64_t {
        int64_t ret = 0;
        FOR(sz(X)) {
            FOR(j, i + 1, sz(X)) { ret += (X[i] + X[j]) * (X[i] + X[j]); }
        }
        return ret;
    };
    int64_t ans = count(A) + count(B);
    cout << ans << '\n';
}
