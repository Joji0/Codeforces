...
void solve() {
        ...
        int64_t L;
        cin >> L;
        auto dijkstra = [&](int source, vector<int64_t> &d) {
                d[source] = 0;
                priority_queue<array<int64_t, 2>, vector<array<int64_t, 2>>, greater<>> pq;
                pq.push({0, source});
                while (!pq.empty()) {
                        auto [dist, node] = pq.top();
                        pq.pop();
                        if (dist != d[node]) continue;
                        for (auto &[nxt, wei] : Graph[node]) {
                                if (chmin(d[nxt], dist + wei)) {
                                        pq.push({d[nxt], nxt});
                                }
                        }
                }
        ...
        auto calc = [&](int u, int v, int64_t w) {
                if (dist[u] < L && dist[u] + w > L) {
                        int64_t take = L - dist[u], other = dist[v] + w - take;
                        if (other > L)
                                ans++;
                        else if (other == L && (dist[u] < dist[v] || (dist[u] == dist[v] && u < v))) {
                                ans++;
                        }
                }
        };
        for (auto &[u, v, w] : Edges) {
                calc(u, v, w), calc(v, u, w);
        }
        cout << ans << '\n';
}
