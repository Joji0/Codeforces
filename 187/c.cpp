...
void solve() {
        ...
        if (!has_t) {
                volunteers.push_back(T);
        }
        auto ok = [&](const int q) -> bool {
                vector<int> dist(N, INF);
                DSU dsu(K + 1);
                vector<int> src(N, -1), id(N, -1);
                queue<int> que;
                int run = 0;
                for (auto &x : volunteers) {
                        dist[x] = 0;
                ...
                while (!que.empty()) {
                        ...
                        for (int &nxt : G[node]) {
                                ...
                                if (dist[nxt] == INF) {
                                        ...
                                        src[nxt] = src[node];
                                        que.push(nxt);
                                } else if (src[nxt] != src[node]) {
                                        if (dist[nxt] + dist[node] + 1 <= q) {
                                                dsu.join(id[src[nxt]], id[src[node]]);
                                        }
                                }
                        }
                }
                return dsu.same(id[S], id[T]);
        };
        int l = 0, r = N + 5, ans = -1;
        while (l <= r) {
                int mid = (l + r) / 2;
