...
void solve() {
    int n, m, r;
    cin >> n >> m >> r;
    vt<vt<vt<int64_t>>> cars(65, vt<vt<int64_t>>(65, vt<int64_t>(65))),
        dp(65, vt<vt<int64_t>>(65, vt<int64_t>(1005)));
    vt<vt<int64_t>> best(65, vt<int64_t>(65));
    FOR(m) {
        FOR(j, n) {
            FOR(k, n) { cin >> cars[i][j][k]; }
        }
        FOR(j, n) {
            FOR(k, n) {
                FOR(l, n) {
                    chmin(cars[i][k][l], {cars[i][k][j] + cars[i][j][l]});
                }
            }
        }
    }
    FOR(n) {
        FOR(j, n) {
            best[i][j] = INF;
            FOR(k, m) { chmin(best[i][j], cars[k][i][j]); }
        }
    }
    FOR(n) {
        FOR(j, n) { dp[i][j][0] = best[i][j]; }
    }
    FOR(k, 1, 1005) {
        FOR(s, n) {
            FOR(t, n) { dp[s][t][k] = INF; }
        }
    }
    FOR(k, 1, 1005) {
        FOR(s, n) {
            FOR(t, n) {
                chmin(dp[s][t][k], dp[s][t][k - 1]);
                FOR(tr, n) {
                    chmin(dp[s][t][k], dp[s][tr][k - 1] + best[tr][t]);
                }
            }
        }
    }
