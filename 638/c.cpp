...
void solve() {
    ...
    function<void(int, int, int)> dfs = [&](int node, int par, int avoid) {
        int startColor = 1;
        EACH(nxt, Tree[node]) {
            if (nxt == par)
                continue;
            if (startColor == avoid)
                startColor++;
            int idx = -1;
            if (id.contains({node, nxt})) {
                idx = id[{node, nxt}];
            } else {
                idx = id[{nxt, node}];
            }
            colors[idx] = startColor;
            dfs(nxt, node, startColor);
            startColor++;
        }
    };
    dfs(1, -1, -1);
    int k = -1;
    FOR(i, 1, n) { chmax(k, colors[i]); }
    cout << k << '\n';
    map<int, vt<int>> mp;
    FOR(i, 1, n) { mp[colors[i]].pb(i); }
    for (auto &[color, info] : mp) {
        cout << sz(info) << " ";
        EACH(in, info) { cout << in << " "; }
        cout << '\n';
    }
}
