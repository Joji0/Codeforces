...
void solve() {
    // The idea is to use dijkstra and if we encounter a node where horse is
    // available, we will use that horse. If on our way we encounter a second
    // horse, from that path we can make Marian stop and Robin start at that
    // particular node using a horse back to node n.
    int n, m, h;
    cin >> n >> m >> h;
    vt<vt<pair<int64_t, int64_t>>> adj(n + 1);
    // dis[node][horse] = minimum distance to node using horse if horse = 1,
    // else not using horse.
    vt<vt<int64_t>> dis(n + 1, vt<int64_t>(2, 1e18));
    vt<bool> horse(n + 1, false);
    FOR(h) {
        int v;
        cin >> v;
        horse[v] = true;
    ...
    // store current distance, current vertex, and horse or non-horse
    pq.push({0, 1, horse[1]});
    dis[1][horse[1]] = 0;
    while (!pq.empty()) {
        auto [dist, node, h] = pq.top();
        pq.pop();
        for (auto &[v, weight] : adj[node]) {
            if (horse[v] && h) {
                // we already have a horse but encounter another one. Pass it
                // for Robin.
            }
            if (chmin(dis[]))
        }
