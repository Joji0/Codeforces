...
void solve() {
        ...
        int rg = 110001, lf = -110001, offset = -lf, siz = rg - lf + 67;
        auto idx = [&](int delta) -> int { return delta + offset; };
        Tensor<int, 2> dp({N + 1, siz}, -INF);
        dp[{0, idx(0)}] = 0;
        bool ok = false;
        for (int i = 1; i <= N; i++) {
                for (int j = lf; j <= rg; j++) {
                        int id = idx(j);
                        int delta = A[i] - K * B[i];
                        if (dp[{i - 1, id}] != -INF) dp[{i, id}] = max(dp[{i, id}], dp[{i - 1, id}]);
                        if (id - delta >= 0 && id - delta < siz && dp[{i - 1, id - delta}] != -INF) {
                                if (chmax(dp[{i, id}], dp[{i - 1, id - delta}] + A[i]) && id == idx(0)) {
                                        ok = true;
                                }
                        }
                }
