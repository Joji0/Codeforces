...
vt<vt<int64_t>> Graph(26, vt<int64_t>(26, INF));
...
void solve() {
    ...
    FOR(k, 26) {
        FOR(i, 26) {
            FOR(j, 26) { chmin(Graph[i][j], Graph[i][k] + Graph[k][j]); }
        }
    }
    int64_t cost = 0;
    string ans(sz(s), 'a');
    FOR(sz(s)) {
        ...
        FOR(tr, 26) {
            char opt = tr + 'a';
            if (Graph[u][tr] != INF && Graph[v][tr] != INF) {
                if (chmin(add, Graph[u][tr] + Graph[v][tr])) {
                    change = opt;
                }
            }
        }
