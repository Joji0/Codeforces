...
void solve() {
    ...
    cin >> n;
    vt<int64_t> c(n);
    vt<vt<int64_t>> dp(n + 1, vt<int64_t>(2, INF));
    vt<string> str(n);
    cin >> c >> str;
    dp[1][0] = 0;
    dp[1][1] = c[0];
    FOR(i, 2, n + 1) {
        FOR(j, 2) {
            FOR(k, 2) {
                string cur = str[i - 1], prev = str[i - 2], revcur, revprev;
                if (j) {
                    revcur = cur;
                    reverse(all(revcur));
                }
                ...
                if (j) {
                    if ((k && revprev <= revcur) || (!k && prev <= revcur)) {
                        chmin(dp[i][j], dp[i - 1][k] + c[i - 1]);
                    }
                } else {
                    if ((k && revprev <= cur) || (!k && prev <= cur)) {
