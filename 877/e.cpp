...
void solve() {
        ...
        struct Lazy {
                int lazy;
                Lazy(int lz = 0) : lazy(lz) {}
        };
        Node ID = Node();
        Lazy LID = Lazy();
        auto merge = [&](const Node &a, const Node &b) -> Node { return Node(a.mask + b.mask); };
        auto make = [&](const int64_t &x) -> Node { return Node(x); };
        auto apply_node = [&](Node &node, const Lazy &lazy, int rg) {
                if (lazy.lazy) node.mask = rg - node.mask;
        };
        auto compose = [&](Lazy &lazy, const Lazy &par) { lazy.lazy ^= par.lazy; };
        auto is_lazy_id = [&](const Lazy &lazy) -> bool { return lazy.lazy == 0; };
        SegmentTree<int, Node, Lazy> seg(N, ID, LID, merge, make, apply_node, compose, is_lazy_id);
        seg.build(flat);
        while (Q--) {
                string t;
                int v;
                ...
                v--;
                if (t == "pow") {
                        seg.update(tin[v], tout[v] - 1, Lazy(1));
                } else {
                        cout << seg.query(tin[v], tout[v] - 1).mask << '\n';
                }
        }
}
