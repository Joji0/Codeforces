...
void solve() {
    // idea : brute force (1<<16) possibilites
    int n, m;
    cin >> n >> m;
    map<string, int> id;
    map<int, string> backId;
    int idx = 0;
    FOR(n) {
        string name;
        cin >> name;
    ...
    set<array<int, 2>> poor;
    FOR(m) {
        string a, b;
        cin >> a >> b;
        poor.insert({id[a], id[b]});
        poor.insert({id[b], id[a]});
    }
    vt<string> team;
    int best = 0;
    FOR(mask, 0, (1 << n)) {
        bool ok = true;
        set<array<int, 2>> check;
        FOR(n) {
            FOR(j, i + 1, n) {
