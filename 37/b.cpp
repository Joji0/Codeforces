...
void solve() {
    int N, maxH, reg, health;
    cin >> N >> maxH >> reg;
    vt<array<int, 2>> scrolls(N);
    FOR(N) { cin >> scrolls[i][0] >> scrolls[i][1]; }
    int64_t t = 0, accum = 0;
    health = maxH;
    vt<int> used(N, -1), order;
    int prev = maxH;
    while (1) {
        health -= accum;
        health = min(health + reg, maxH);
        if (health <= 0) {
            break;
        }
        int getDamage = -1, idx = -1;
        FOR(N) {
            if (health * 100 <= scrolls[i][0] * maxH && used[i] == -1 &&
                getDamage < scrolls[i][1]) {
                getDamage = scrolls[i][1];
                idx = i;
            }
        }
    ...
    cout << "YES\n";
    cout << t << " " << sz(order) << '\n';
    EACH(idx, order) { cout << used[idx] << " " << idx + 1 << '\n'; }
}
