...
void solve() {
    // The idea is to use the fact that if we place the trap at point P then the
    // frogs that will be captured at this point are those with a[i], such that
    // a[i] | P, i.e., P % a[i] = 0. We can just then make a frequency array of
    // size n which captures the count of each a[i]. After that, we make another
    // frequency array which this time, captures the number of frog for each [1,
    // n] points. This way we can avoid TLE because we use a frequency array to
    // help another frequency array.
    int n;
    cin >> n;
    vt<int> a(n);
    cin >> a;
    int freq[200005], capturepoints[200005];
    memset(freq, 0, sizeof(freq));
    memset(capturepoints, 0, sizeof(capturepoints));
    FOR(n) {
        if (a[i] < 200005)
            freq[a[i]]++;
    }
    FOR(i, 1, n + 1) {
        FOR(j, i, n + 1, i) { capturepoints[j] += freq[i]; }
    }
    int ans = 0;
    FOR(i, 1, n + 1) { chmax(ans, capturepoints[i]); }
    cout << ans << '\n';
}
