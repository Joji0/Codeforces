...
void solve() {
    ...
    len[0] = 1e12;
    FOR(i, 1, x + 1) {
        len[i] = len[i - 1] - (len[i - 1] / y);
        if (len[i] == len[i - 1]) {
            FOR(j, i + 1, x + 1) { len[j] = len[i]; }
            break;
        }
    }
    if (len[x] < k) {
    ...
    auto f = [&](int64_t idx) -> int64_t { return idx - idx / y; };
    // coba mapping ke state sebelum removal
    int64_t ans = k;
    FOR(i, x, 0, -1) {
        int64_t l = 1, r = len[i - 1], curr = -1;
        while (l <= r) {
            int64_t mid = (l + r) / 2;
            if (f(mid) >= ans) {
                curr = mid;
