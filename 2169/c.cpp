...
void solve() {
    int n;
    cin >> n;
    vt<int64_t> A(n + 1, 0), pref(n + 1, 0), prefL;
    FOR(i, 1, n + 1) cin >> A[i];
    FOR(i, 1, n + 1) { pref[i] = pref[i - 1] + A[i]; }
    int64_t L = -1e18, gain1 = 0, gain2 = 0;
    FOR(i, 1, n + 1) { chmax(gain1, 2LL * i - A[i]); }
    FOR(r, 2, n + 1) {
        int64_t l = r - 1, currL = pref[l - 1] - 1LL * l * l + l,
                currR = 1LL * r * r + r - pref[r];
        chmax(L, currL);
        int64_t curr = currR + L;
        chmax(gain2, curr);
    }
    cout << accumulate(all(A), 0LL) + max(gain1, gain2) << '\n';
}
