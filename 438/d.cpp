...
struct SegmentTree {
    ...
    void modOp(int v, int tl, int tr, int l, int r, int64_t x) {
        if (t[v].mx < x)
            return;
        if (tl == tr) {
            t[v].v %= x;
        ...
        int tm = (tl + tr) / 2;
        if (l <= tm) {
            modOp(v * 2, tl, tm, l, min(r, tm), x);
        }
        if (r > tm) {
            modOp(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, x);
        }
        t[v].v = t[v * 2].v + t[v * 2 + 1].v;
        t[v].mx = max(t[v * 2].mx, t[v * 2 + 1].mx);
    };

    void modOp(int l, int r, int64_t x) { modOp(1, 0, n - 1, l, r, x); }
};
...
void solve() {
    ...
    FOR(m) {
        ...
        if (type == 1) {
            int l, r;
            cin >> l >> r;
            cout << segT.sum(l - 1, r - 1) << '\n';
        } else if (type == 2) {
            int l, r;
            int64_t x;
            cin >> l >> r >> x;
            segT.modOp(l - 1, r - 1, x);
        } else {
            int k;
            int64_t x;
            cin >> k >> x;
