...
int64_t dp[20][10][10][2];

int64_t solve(int pos, int first, int last, int tight, const vt<int> &digits) {
    if (pos == sz(digits)) {
        return (first != -1 && first == last);
    }
    if (dp[pos][first][last][tight] != -1) {
        return dp[pos][first][last][tight];
    }
    int64_t ans = 0;
    ...
    FOR(lim + 1) {
        ...
        if (first == 0) {
            if (i) {
                ans += solve(pos + 1, i, i, newTight, digits);
            } else {
                ans += solve(pos + 1, 0, 0, newTight, digits);
            }
        } else {
            ans += solve(pos + 1, first, i, newTight, digits);
        }
    }
    return dp[pos][first][last][tight] = ans;
}

int64_t f(int64_t x) {
    vt<int> digits;
    string s = to_string(x);
    EACH(ch, s) { digits.pb(ch - '0'); }
    memset(dp, -1, sizeof(dp));
    return solve(0, 0, 0, 1, digits);
}
